...
for i in x:
    if i == 0:
        an.e_analyzer(input_path)
    if i == 1:
        an.p_analyzer(input_path)
    if i == 2:
        an.multiple_p_analyzer(input_path)
    if i == 3:
        an.NODF_analyzer(input_path)
    if i == 4:
        an.precision_analyzer(input_path)
    if i == 5:
        an.S_mean_precision(input_path+'/precision')
print('Test analyzer effettuato con successo.')
